"""
ScyllaSOTA: Scylla tokenizer + Track A SOTA stack.

Merges:
  - Scylla (998-token TokenMonster vocab) + Full Hessian GPTQ + XSA-11 + FA3 + coprime loader
  - 3-Layer Depth Recurrence (layers 3-5, activates at 35% of training)
  - Parallel Residuals (layers 7-9, GPT-J style)
  - QK-Gain 5.25
  - Legal Score-First TTT (SGD lr=0.01, 3 epochs/chunk)
  - Fixed byte accounting in candidate.meta.npz (86 tokens, C/D/W markers)
...
class Hyperparameters:
    ...
    # Architecture
    qk_gain_init = float(os.environ.get("QK_GAIN_INIT", 5.25))   # Track A optimal
    ...
    # Depth recurrence (Track A)
    num_loops = int(os.environ.get("NUM_LOOPS", 2))
    loop_start = int(os.environ.get("LOOP_START", 3))
    loop_end = int(os.environ.get("LOOP_END", 5))
    enable_looping_at = float(os.environ.get("ENABLE_LOOPING_AT", 0.35))
    # Parallel residuals (Track A)
    ...
    # TTT - lr bumped to 0.01 (Scylla found TTT neutral at 0.9485; retry with lr=0.01)
...
# --- Parallel Muon optimizer (overlapped reduce-scatter / all-gather) ---
...
class Block(nn.Module):
    def __init__(
        ...
        # Parallel residuals flag (Track A, GPT-J style): set by GPT.__init__
        self.parallel = False
    ...
    def forward(
        ...
        normed_in = self.attn_norm(x_in) * self.ln_scale_factor
        attn_out = self.attn(normed_in, q_w, k_w, v_w, out_w, v_embed=v_embed)
        if self.parallel:
            # GPT-J style: both attention and MLP read from same pre-norm state x_in
            mlp_normed_in = self.mlp_norm(x_in) * self.ln_scale_factor
            mlp_out = self.mlp_scale.to(dtype=x_in.dtype)[None, None, :] * self.mlp(mlp_normed_in, up_w, down_w)
            x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out + mlp_out
        else:
            x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
            mlp_normed = self.mlp_norm(x_out) * self.ln_scale_factor
            mlp_out = self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(mlp_normed, up_w, down_w)
            x_out = x_out + mlp_out
        return x_out
...
class GPT(nn.Module):
    def __init__(
        ...
        # --- Depth recurrence indices (Track A) ---
        ...
        # num_skip_weights must handle both looping and non-looping modes
        ...
        self.skip_gates = (
            ...
            if skip_gates_enabled else None
        )

        # looping_active: toggled at 35% of training
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    layer_idx=i, ln_scale=ln_scale, neg_slope=neg_slope,
                )
        ...
        if rope_dims > 0:
            head_dim_rope = model_dim // num_heads
            for block in self.blocks:
                block.attn.rope_dims = rope_dims
                block.attn.rotary = Rotary(head_dim_rope, base=rope_base, train_seq_len=1024, rope_dims=rope_dims)
        ...
        if self.ve_layer_indices:
            ...
            self.ve_shared = None
            self.ve_layer_scales = nn.ParameterList()
        self.value_embeds = nn.ModuleList()  # compat
        ...
        # Parallel residuals on layers >= parallel_residual_start (Track A)
    ...
    def _run_layers(self, x: Tensor, x0: Tensor, input_ids: Tensor) -> Tensor:
        """Run encoder + decoder layers, handling depth recurrence when looping_active."""
        n = self.num_layers
        ve_cache: dict = {}
        skips: list[Tensor] = []

        # Choose encoder/decoder indices based on looping state
        enc_iter = self.encoder_indices if self.looping_active else range(self.num_encoder_layers)
        dec_iter = self.decoder_indices if self.looping_active else range(self.num_encoder_layers, n)
...
def eval_val_sliding_ttt(
    ...
    """Legal score-first TTT: score each chunk first, then train. Every token scored BEFORE update."""
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    consumed = set()
    for i in range(n):
        for key, lst, offset in [
            (f"blocks.{i}.attn.c_q.weight", qo_slices, 0),
            (f"blocks.{i}.attn.proj.weight", qo_slices, n),
            (f"blocks.{i}.attn.c_k.weight", kv_slices, 0),
            (f"blocks.{i}.attn.c_v.weight", kv_slices, n),
            (f"blocks.{i}.mlp.fc.weight", up_slices, 0),
            (f"blocks.{i}.mlp.proj.weight", down_slices, 0),
        ]:
            if key in sd:
                lst[offset + i] = sd[key]
                consumed.add(key)
...
def _do_warmup_steps(
    ...
    for warmup_step in range(args.warmup_steps):
        ...
        if args.warmup_steps <= 20 or (warmup_step + 1) % 10 == 0 or warmup_step + 1 == args.warmup_steps:
            log0(f"{label}_step:{warmup_step + 1}/{args.warmup_steps}")
...
def main() -> None:
    ...
    optimizer_tok = torch.optim.AdamW(
        tok_params, betas=(args.beta1, args.beta2), eps=args.adam_eps, weight_decay=args.adam_wd, fused=True,
    )
    optimizer_muon = Muon(
    ...
    par_layers = [i for i, b in enumerate(base_model.blocks) if b.parallel]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"parallel_residuals:layers {par_layers}")
    log0(f"depth_recurrence:num_loops={args.num_loops} loop_start={args.loop_start} loop_end={args.loop_end} "
         f"enable_at={args.enable_looping_at}")
    if args.num_loops > 0:
        log0(f"depth_recurrence:encoder_indices={base_model.encoder_indices}")
        log0(f"depth_recurrence:decoder_indices={base_model.decoder_indices}")
    log0(f"skip_weights:{base_model.num_skip_weights} skip_gates:{base_model.skip_gates is not None}")
    log0(f"qk_gain_init:{args.qk_gain_init}")
    ...
    def training_frac(elapsed_ms: float, step: int) -> float:
    ...
    if args.warmup_steps > 0:
        ...
        _do_warmup_steps(args, model, base_model, optimizers, train_loader,
                         grad_accum_steps, grad_scale, distributed, log0, label="warmup")
        ...
        # Loop warmup: prime the recurrence path (Track A double-warmup)
        if args.num_loops > 0:
            base_model.looping_active = True
            log0(f"loop_warmup:enabled encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
            _do_warmup_steps(args, model, base_model, optimizers, train_loader,
                             grad_accum_steps, grad_scale, distributed, log0, label="loop_warmup")
    ...
    while True:
        ...
        # --- Enable depth recurrence at enable_looping_at fraction ---
        if args.num_loops > 0 and not base_model.looping_active:
            frac = training_frac(elapsed_ms, step)
        ...
        muon_momentum = (1 - frac_m) * args.muon_momentum_warmup_start + frac_m * args.muon_momentum
        for group in optimizer_muon.param_groups:
    ...
    # GPTQ calibration: base_model.looping_active should be True at end of training
    gptq_hessians = None
    if args.use_gptq:
        t_gptq = time.perf_counter()
        log0(f"gptq:calibrating with {args.gptq_calib_samples} batches (looping_active={base_model.looping_active})...")
    ...
    # Enable looping for eval (if trained with recurrence)
    if args.num_loops > 0:
        eval_model.looping_active = True
        log0(f"eval_model:looping_active=True encoder:{eval_model.encoder_indices}")
